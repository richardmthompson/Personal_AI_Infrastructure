...
## Overview
The PAI Voice System provides text-to-speech capabilities for the primary assistant and all agents using ElevenLabs TTS API. The system uses a unified stop-hook approach where completion messages are read aloud with distinct voices for the assistant and each specialized agent, providing voice differentiation through character-matched, high-quality ElevenLabs voices.
...
1. **Task Execution**
   ...
   - Either the assistant handles it directly or delegates to an agent
...
4. **Voice Selection**
   - For assistant's completions: Uses the configured primary assistant voice ID
...
|--------|---------------------|-------------------|
| Assistant | s3TPKV1kjDlVtZbl4Ksh | Professional, conversational |
...
ELEVENLABS_VOICE_ID="s3TPKV1kjDlVtZbl4Ksh"  # Optional, default assistant voice
```
...
**Main Assistant:** `${PAI_DIR}/hooks/stop-hook.ts`
```typescript
voice_id: 's3TPKV1kjDlVtZbl4Ksh'  // Primary assistant's ElevenLabs voice ID
```

**Subagents:** `${PAI_DIR}/hooks/subagent-stop-hook.ts`
```typescript
const ELEVENLABS_VOICE_IDS: Record<string, string> = {
  'assistant': 's3TPKV1kjDlVtZbl4Ksh',
...
### Required Format
Every response from the assistant or an agent must end with:
...
```bash
# Test assistant's voice
curl -X POST http://localhost:8888/notify \
  -H "Content-Type: application/json" \
  -d '{"message":"Testing assistant voice","voice_id":"s3TPKV1kjDlVtZbl4Ksh"}'
...
- ✅ Distinct voices for assistant and each agent
...
Voice makes the assistant and agents feel more alive and engaging!
